...
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenEmptyMap_AfterAddingElements_MapIsBalanced,
                              K,
                              TestedKeyTypes)
{
  Map<K> map;
  ...
  BOOST_CHECK(map.end().getCnode()->left->height < 8);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(GivenNonEmptyMap_HeightIsCorrect,
                              K,
                              TestedKeyTypes)
{
  Map<K> map1 = { { 1, "" } };
  Map<K> map2 = { { 2, "" }, { 1, "" }, { 3, "" } };
  Map<K> map3 = { { 2, "" }, { 1, "" }, { 3, "" }, { 4, "" } };
  BOOST_CHECK( (map1.end().getCnode()->left->height == 1) && (map2.end().getCnode()->left->height == 2) && (map3.end().getCnode()->left->height == 3));
}
